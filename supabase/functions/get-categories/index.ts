import { serve } from "https://deno.land/std@0.168.0/http/server.ts"
import { JWT } from "https://esm.sh/google-auth-library@8.7.0"

const SERVICE_ACCOUNT = {
    "type": "service_account",
    "project_id": "app-finanzas-485816",
    "private_key_id": "89f226f88f020b2a0de085258de27fd73d81dd87",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "aa-526@app-finanzas-485816.iam.gserviceaccount.com",
};

const SPREADSHEET_ID = "1sQRRvJw2WQGwM0Drr77Lz5LHHdhyzC8XEjJYBWMXdJI";

serve(async (req: Request) => {
    try {
        console.log('--- START get-categories ---');
        const client = new JWT({
            email: SERVICE_ACCOUNT.client_email,
            key: SERVICE_ACCOUNT.private_key,
            scopes: ["https://www.googleapis.com/auth/spreadsheets.readonly"],
        });

        console.log('Authorizing JWT...');
        await client.authorize();
        console.log('JWT Authorized Successfully');

        const range = "Listas desplegables!A2:H100";
        const url = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(range)}`;

        console.log('Fetching from URL:', url);
        const response = await fetch(url, {
            headers: { Authorization: `Bearer ${client.credentials.access_token}` },
        });

        if (!response.ok) {
            const errText = await response.text();
            console.error('Google API Error:', response.status, errText);
            throw new Error(`Google API responded with ${response.status}: ${errText}`);
        }

        const data = await response.json();
        console.log('Data received from Google Sheets');

        if (!data.values) {
            console.warn('No values found in the specified range');
            throw new Error("No data found in 'Listas desplegables'. Check if the tab name is exact.");
        }

        // Listas desplegables mapping:
        // A: Meses | B: Categorías Egreso | D: Categorías Ingreso | F: Categorías Ahorros
        const categories = {
            months: data.values.map((row: any[]) => row[0]).filter(Boolean),
            expenses: data.values.map((row: any[]) => row[1]).filter(Boolean),
            income: data.values.map((row: any[]) => row[3]).filter(Boolean),
            savings: data.values.map((row: any[]) => row[5]).filter(Boolean),
        };

        console.log('Successfully parsed categories');
        return new Response(JSON.stringify(categories), {
            headers: { "Content-Type": "application/json" },
            status: 200,
        });
    } catch (err: any) {
        console.error('CRITICAL ERROR in get-categories:', err.message);
        return new Response(JSON.stringify({
            error: err.message,
            details: "Check if the spreadsheet has been shared with 'aa-526@app-finanzas-485816.iam.gserviceaccount.com' as Editor."
        }), {
            headers: { "Content-Type": "application/json" },
            status: 500,
        });
    }
});
