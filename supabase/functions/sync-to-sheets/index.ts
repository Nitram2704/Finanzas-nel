...
serve(async (req: Request) => {
    try {
        const { date, month, category, amount, description, type } = await req.json();
        ...
        const range = `${TYPE_TO_SHEET[type] || 'Registro Egresos'}!A:E`;
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                values: [[date, month, category, amount, description]],
            }),
        });
